...
public class CommentRestController {
	...
	{
		log.debug("Starting of method create Comment");
	...
	{
		log.debug("Starting of method update Comment");
		...
		if(comments==null)
		{
			log.debug("Starting of comment is null method of update Comment");
		...
		else
		{
			...
			{
				log.debug("Starting of nested if method of else method of updateBlog");
				long d = System.currentTimeMillis();
				Date today = new Date(d);
				...
				if(newComment.getCommentsmsg()==null || newComment.getCommentsmsg()=="")
				{
					newComment.setCommentsmsg(comments.getCommentsmsg());
				}
			...
			{
				log.debug("Starting of else method of update comments");
	...
	public ResponseEntity<Comments> deleteComments(@RequestBody Comments newComment)
	{
		...
		else
		{
			if(comments.getId().equals(loggedInUserId))
			{
				log.debug("Starting of nested if method of else method of Delete Comment");
	...
	public ResponseEntity<List<Comments>> displayAllCommentsByBlog(@PathVariable("blogid")int blogid)
	{
		List<Comments> commentslist = commentDAO.getCommentsListByBlog(blogid);
		return new ResponseEntity<List<Comments>>(commentslist,HttpStatus.OK);
	}
	
	@GetMapping("/CommentsListByForum/{id}")
	public ResponseEntity<List<Comments>> displayAllCommentsByForum(@PathVariable("forumid")int forumid)
	{
		List<Comments> commentslist = commentDAO.getCommentsListByForum(forumid);
		return new ResponseEntity<List<Comments>>(commentslist,HttpStatus.OK);
	}
